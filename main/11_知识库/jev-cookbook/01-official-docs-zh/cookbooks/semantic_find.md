...
1. 给每行打上一个 ID，让 TypeSafe 能够指向它。
2. 用一个 `Choice` 问题按各行回答查询的程度为这些行 ID 排序。Choice 问题的概率总和恒为 1，因此即使没有任何行回答查询，也会有一行排第一。
3. 在同一请求中，用一个 `Noul` 问题检查文档究竟是否包含答案。
...
`JsonCache` 会重放随附的 API 响应，因此下面的步骤无需 API 密钥、不产生任何花费即可运行。若要改为实时发起请求，请设置 `TYPESAFE_API_KEY` 并删除 `json_cache.json`。
...
缓存避免了重复下载，`splitlines()` 得到一个包含 218 个字符串的列表。
...
<Info> 一个 `Choice` 问题最多接受 255 个选项，因此这个配方可以在一次请求中搜索最多 255 行的文档。超过这个数量，就分两遍搜索：一个 Choice 问题选出一个行窗口，第二个问题对窗口内的行进行排序。 </Info>
...
这些阈值能区分下面的示例，但在用于生产环境之前，请对照你自己的文档对它们进行调优。
...
* **仲裁：** 排序给最接近的行打了 0.86 分，但 `exists` 只有 0.14。答案不在文档中。
* **监护人许可：** 年龄规则排第一，但它没有回答监护人许可是否会改变这条规则。结果是**部分涉及**。
