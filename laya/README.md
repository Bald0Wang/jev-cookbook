...
资料整理：2026-09-23。模型说明以 [ModelScope 模型卡](https://modelscope.cn/models/convaiinnovations/laya) 和 [Laya 源码仓库](https://github.com/NandhaKishorM/laya) 为主；横向评测采用 JevBench v1.2。文中三篇公众号文章均已给出公开链接，并在文中区分作者主张、社区设备实测和独立基准结果。

> **关于本目录的上下文**：本文原为 Jev 知识库（jev-cookbook 归档）中的一节，随本节一并迁入本仓库。
> 文中引用的公众号文章均给出公开原文链接，无需本地快照即可核对；模型权重（约 2.2 GB）不随仓库分发，
> 获取方式见第 5 节。
...
- 《Laya 开源：比Jev快4倍！421M 参数，33 毫秒完成 System 1 决策》（[公开原文](https://mp.weixin.qq.com/s/9SJf3nhK25rZcZwQNTg7mw)）解释了 ModernBERT-large、`[MASK]` 候选标记、双层决策头与 RLCD 训练。标题和文中的“快 4 倍”是作者按不同来源延迟作出的宣传性比较，不是同一硬件、同一网络路径的复现实验。
- 《4.6k star，开源版Jev悄悄发布》（[公开原文](https://mp.weixin.qq.com/s/KgBXK1PEuswkfk4xXcrN0A)）引用的模型卡宣传对比包括快 7.8 倍、准确率高 3.9 个百分点、校准好 3 倍；同文也说明 Jev 数据来自第三方、不是同一环境复测。限制数据包括：英语 / 多语言基础 checkpoint 在 typed-decisions 零样本集准确率 0.362 / 0.342，低于多数类基线 0.461；微调后 0.766 来自该基准的训练切分；Banking77 高基数分类 Jev 0.870、Laya 0.425；软分布匹配 Jev 0.580、Laya 0.471；英语 checkpoint 在高棉语曾报告 0 准确率、0.952 置信度。应把这些数看成特定数据与测量口径下的风险提示，而不是统一排名。
- 《「开源版Jev」登上Hugging Face热榜第一》（[公开原文](https://mp.weixin.qq.com/s/39esHoH-GVpfdXjunng1oA)）汇总了 `laya-mlx` 社区报告：设备内存约 1 GB；M3 Max 贪吃蛇最高 60 次决策 / 秒；Core ML 功耗最高降低 64%；另一组蛇游戏报告 Laya 86.5 次 / 秒、P50 9 ms，Jev 3.2 次 / 秒、API 往返约 317 ms；M5 Pro 中位数 15.3 ms 对 298.1 ms；俄罗斯方块约 11 倍。这些是特定硬件、任务和 MLX / Core ML 运行时的社区结果；不能直接等同于本项目 PyTorch MPS 服务的吞吐，也不能视为同模型同部署条件的质量比较。

## 4. 微调与数据构建

当前工作区含 Laya checkpoint、推理代码、CUDA head-only trainer 和试跑数据。按[微调实操指南](FINETUNING.md)可以下载多语言权重、检查 JSONL、运行微调和查看训练曲线；也可逐格运行[中文 GPU 微调 Notebook](notebooks/zh_head_finetuning.ipynb)。需要合成训练候选时，可参考 [DeepSeek 数据生成指南](DATA_GENERATION.md)；生成记录必须审核后才能进入正式 train。随仓库提供的合成试跑样本不是人工 gold，不代表生产效果。
...
```bash
laya/.venv/bin/python -m pip install -r laya/requirements.txt
USE_TF=0 LAYA_DEVICE=mps LAYA_MODEL_DIR="$PWD/laya/models/multilingual" \
  laya/.venv/bin/python -m laya.serve --host 127.0.0.1 --port 8811
```
...
在 Jev Cookbook 工作区根目录打开 PowerShell：
...
$env:USE_TF = "0"
$env:LAYA_MODEL_DIR = (Resolve-Path ".\laya\models\multilingual").Path
...
- 本地评测：[JevBench v1.2](https://github.com/fstandhartinger/jevbench/blob/main/RESULTS-v1.2.md) 与 [测量方法 / 系统映射](https://github.com/fstandhartinger/jevbench/blob/main/docs/v1.2-additions.md)。分数为综合评分，备注区分硬件、来源、研究预览与估算成本。
- 三篇公众号文章的用户原始链接：[ModelScope 社区的架构介绍](https://mp.weixin.qq.com/s/9SJf3nhK25rZcZwQNTg7mw)、[PaperAgent 的开源发布与限制梳理](https://mp.weixin.qq.com/s/KgBXK1PEuswkfk4xXcrN0A)、[机器之心的社区热度及 Apple 运行时测试](https://mp.weixin.qq.com/s/39esHoH-GVpfdXjunng1oA)。上述“本地归档文章”小节对三者作了来源归属与证据级别说明。
