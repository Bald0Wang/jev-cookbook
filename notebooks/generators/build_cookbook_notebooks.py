...
def build_merged():
    """18 篇菜谱合并为单一章节 08_实战指南：共享一套准备样板，每篇一节。"""
    built = []
    for slug, (title, source, purpose, builder) in BUILDERS.items():
        nb = make_notebook(title, source, purpose, builder)
        built.append((slug, title, source, purpose, nb))

    cells = []
    first_nb = built[0][4]
    # 共享准备：跳过第一篇自己的标题格，取 0.准备 的 9 格
    cells.extend(first_nb.cells[1:10])

    # 前八篇：与共享样板同一套 API（TS.call），只保留内容格
    for i, (slug, title, source, purpose, nb) in enumerate(built, start=1):
        header = (f"## 第 {i} 篇 · {title}\n\n"
                  f"对应官方 Cookbook：[{title}]({source})。{purpose}\n\n"
                  "---")
        cells.append(nbf.v4.new_markdown_cell(header))
        for cell in nb.cells[10:-1]:  # 尾部小结是样板重复，丢弃
            if cell.cell_type == "markdown":
                cell = nbf.v4.new_markdown_cell(_demote(cell.source))
            cells.append(cell)

    # 后十篇：独立样板（实例式 ts.call + 各自离线数据），仅去重 pip 安装；
    # 官方目录顺序恰好让它们排在八篇之后，TS 重定义不影响前文已执行的节。
    DISK = ["entity_alignment", "classifying_rag_passages", "citation_check",
            "llm_guardrails", "sde_cascade", "date_extraction",
            "pre_parsed_value_extraction", "hierarchical_classification",
            "autoresearch_feature_discovery", "classification_using_confidence"]
    for j, slug in enumerate(DISK, start=len(built) + 1):
        path = ROOT / "cookbooks" / f"{slug}_experiments.ipynb"
        disk_nb = nbf.read(path, as_version=4)
        texts = [c.source for c in disk_nb.cells]
        title = texts[0].lstrip("# ").splitlines()[0].strip()
        cells.append(nbf.v4.new_markdown_cell(
            f"## 第 {j} 篇 · {title}\n\n"
            f"对应官方 Cookbook：[官方原文](https://docs.typesafe.ai/cookbooks/{slug})。"
            "本篇保留自带的准备样板（含本篇离线示例数据），与前面各节互不共享状态。\n\n---"))
        for cell in disk_nb.cells[1:]:
            t = cell.source
            if cell.cell_type == "markdown":
                if t.startswith("### 0.1") or t.startswith("## 0. 准备"):
                    continue  # 标题与安装说明去重
                cell = nbf.v4.new_markdown_cell(_demote2(t))
            elif cell.cell_type == "code" and t.strip().startswith("%pip"):
                continue
            cells.append(cell)

    cells.append(nbf.v4.new_markdown_cell("""## 小结

十八篇菜谱共享同一个骨架：**TypeSafe 只负责受限、可编程的判断**；排序、阈值、分组、重建文本和函数分派全部由 Python 代码完成。修改任何一节的输入或问题后重新运行，就能观察「模型答案 → 确定性代码」的变化。"""))

    head = nbf.v4.new_markdown_cell("""# 08 · 实战指南：官方 Cookbook 十八篇（Cookbook Lab）
...
配套源文件：`generators/build_cookbook_notebooks.py`（由 18 个独立 notebook 合并生成）。""")
    nb = nbf.v4.new_notebook(cells=[head] + cells)
    nb.metadata["kernelspec"] = {"display_name": "Python 3", "language": "python", "name": "python3"}
    nb.metadata["language_info"] = {"name": "python", "version": "3.12"}
    out = ROOT / "08_实战指南.ipynb"
