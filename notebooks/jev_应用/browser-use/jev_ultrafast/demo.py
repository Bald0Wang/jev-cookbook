...
def command(name, body):
    ...
    if name == "reset":
        ...
        if not goal or len(goal) > 2000:
            raise ValueError("请输入 1–2000 个字符的任务")
        close_browser()
        AGENT = Agent(
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if (
            ...
            return self.send(403, json.dumps({"error": "仅允许本机演示请求"}))
        if not LOCK.acquire(blocking=False):
            return self.send(409, json.dumps({"error": "已有浏览器步骤正在执行"}))
        try:
            ...
            self.send(500, json.dumps({"error": "本地演示失败，不会自动重试。请重新开始以恢复。"}))
        finally:
            LOCK.release()
